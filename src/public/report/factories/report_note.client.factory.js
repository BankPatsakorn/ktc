angular.module('report').factory('reportNoteFactory', ['$rootScope', '$iService',
  ($rootScope, $iService) => {
    return {
      getDataReportSingle: (data, callback) => {
        if ($rootScope.authentication.token) {
          ...
          $iService.postByHeader(`${$iService.url()}get_report_note_italthai`, strJson, header, (res) => {
          // $iService.postByHeader(`${$iService.url()}get_report_note_details`, strJson, header, (res) => {
            if ($.trim(res)) {
              callback(res)
            } else {
              callback([])
