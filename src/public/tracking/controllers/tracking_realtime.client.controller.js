angular.module('tracking').controller('TrackingRealtimeController', ['$rootScope', '$scope', '$timeout', '$interval', '$compile', '$iService', 'trackingRealtimeFactory', 'settingVehicleFactory',
  ($rootScope, $scope, $timeout, $interval, $compile, $iService, trackingRealtimeFactory, settingVehicleFactory) => {
        ...
        trackingRealtimeFactory.getTrackingRealtime(res => {
          if (res.length > 0) {
            ...
            $scope.dataTrackingRealtime = res.map(data => {
              const _modem_id = $scope.dataSettingVehicle.map(_data => (_data.modem_id))
              const _setting = {
                calibrate_working_hour: 0,
                set_point_1: 0,
                set_point_2: 0,
                set_point_3: 0
              }
              if (_modem_id.length > 0) {
                const _index = _modem_id.indexOf(data.modem_id)
                _setting.calibrate_working_hour = parseInt($scope.dataSettingVehicle[_index].calibrate_working_hour)
                _setting.set_point_1 = parseInt($scope.dataSettingVehicle[_index].set_point_1)
                _setting.set_point_2 = parseInt($scope.dataSettingVehicle[_index].set_point_2)
                _setting.set_point_3 = parseInt($scope.dataSettingVehicle[_index].set_point_3)
              }
              Object.assign(data, _setting)
    ...
    $scope.sendNote = (note, modem_id, type) => {
      if (note.hasOwnProperty('message_service') || note.hasOwnProperty('message_repair') || note.hasOwnProperty('message_consult')) {
        if ((typeof note.message_service !== "undefined" && note.message_service != null && note.message_service != "") ||
          (typeof note.message_repair !== "undefined" && note.message_repair != null && note.message_repair != "") ||
          (typeof note.message_consult !== "undefined" && note.message_consult != null && note.message_consult != "")) {
          let _data = {
            modem_id: modem_id,
            type: type
          }
          if (type == 'service') {
            _data = {
              ..._data,
              message: note.message_service
            }
          } else if (type == 'repair') {
            _data = {
              ..._data,
              message: note.message_repair
            }
          } else if (type == 'consult') {
            _data = {
              ..._data,
              message: note.message_consult
            }
          }
          trackingRealtimeFactory.setNote(_data, res => {
            if (res.success) {
              note.message_service = ""
              note.message_repair = ""
              note.message_consult = ""
              $iService.toggleModalMessage({
      ...
      if (!$scope.ListDetail) {
        ...
        if (data.vehicle_name) {
          if (data.vehicle_name.indexOf($scope.ListDetail) != -1 ||
            ...
            data.company_name.indexOf($scope.ListDetail) != -1 ||
            data.customer_name.indexOf($scope.ListDetail) != -1 ||
            data.customer_phone.indexOf($scope.ListDetail) != -1 ||
